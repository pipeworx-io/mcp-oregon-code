...
/**
 * Oregon Revised Statutes — state statutes by citation.
 *
 * Chapter files, section citations: ORS 163.115 lives inside ors163.html
 * alongside every other section of chapter 163 (223,271 characters). The
 * chapter is the part of the citation before the dot, so the file is
 * derivable; the section is cut out of it, bounded by the next section
 * heading. Returning the chapter would answer a one-section question with a
 * hundred statutes.
 *
...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Oregon Revised Statutes');
}

const BASE = 'https://www.oregonlegislature.gov/bills_laws/ors/ors';
const UA = 'pipeworx-mcp-oregon-code/1.0 (+https://pipeworx.io)';

const tools: McpToolExport['tools'] = [{
  name: 'or_revised_statute',
  description:
  ...
  inputSchema: {
    type: 'object',
    properties: {
      section: { type: 'string', description: 'Section as cited, e.g. "163.115" (murder 2) or "813.010" (DUII). Chapter is the part before the dot.' },
    },
    required: ['section'],
  },
}];
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  if (name !== 'or_revised_statute') throw new Error(`Unknown tool: ${name}`);
  const raw = String(args.section ?? '').trim().replace(/\s+/g, '').replace(/^ORS/i, '');
  const m = /^(\d+[A-Za-z]?)\.(\d+)$/.exec(raw);
  if (!m) {
    return { found: false, reason: 'unparsed_section',
      message: `"${args.section}" is not an Oregon Revised Statutes citation.`,
      hint: 'ORS sections are chapter.section, e.g. "163.115" (murder 2) or "813.010" (DUII).' };
  }
  const chapter = m[1];
  ...
  if (res.status === 404) {
    return { found: false, reason: 'chapter_not_found', section: raw, chapter,
      message: `No chapter ${chapter} in the Oregon Revised Statutes (HTTP 404).`,
      hint: 'Chapter numbers are not contiguous — repealed chapters are normal.' };
  }
  if (!res.ok) throw new Error(`Oregon Legislature returned HTTP ${res.status} for ${raw}`);
  ...
  return {
    citation: `ORS ${raw}`, section: raw, chapter,
    text: body, text_chars: body.length, url,
    source: 'Oregon Legislature (oregonlegislature.gov), keyless',
    attribution: 'Oregon statutes are public record.',
  };
}
